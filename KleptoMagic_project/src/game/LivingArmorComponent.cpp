...
void LivingArmorComponent::initComponent() {
	...
	_playerTransform = mngr->getComponent<Transform>(mngr->getHandler(ecs::hdlr::PLAYER));
...
void LivingArmorComponent::update() {
	...
	// Comprobar si la armadura es vulnerable al jugador
	isVulnerable(_playerTransform->getPos());
...
void LivingArmorComponent::render() {
	// Dibuja un cuadrado en la posición de la armadura
